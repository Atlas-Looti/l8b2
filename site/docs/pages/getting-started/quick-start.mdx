...
- `rootDir`: Directory for documentation (default: ".")
- `title`: Title of your documentation site
...
- `pnpm run dev` or `l8b dev`: Run development server with HMR (Hot Module Replacement)
- `pnpm run build` or `l8b build`: Build project for production (output to `.l8b/`)
- `pnpm run preview` or `l8b preview`: Preview production build
...
1. **Explore LootiScript** - Learn more about the LootiScript language in [LootiScript Programming](/looti-script)
2. **API Reference** - See all available APIs in [API Reference](/api)
