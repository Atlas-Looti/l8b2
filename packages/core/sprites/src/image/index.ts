...
export class Image {
	...
	constructor(
		widthOrImage: number | HTMLImageElement | HTMLCanvasElement,
		height?: number,
		...
		if (typeof widthOrImage === "object" && "naturalWidth" in widthOrImage) {
			...
			typeof widthOrImage === "object" &&
			"getContext" in widthOrImage
		) {
			// HTMLCanvasElement
	...
	setDrawScale(
		...
		object_scale_y: number = object_scale_x,
	): void {
		this.initContext();
	...
	drawSpritePart(
		...
		this.initContext();
		SpriteRenderingOps.drawSpritePart(
			this._context!,
			this._state,
			sprite,
			sx,
	...
	drawMap(
		map: MapRenderingOps.MapSource | string,
		x: number,
		y: number,
		w: number,
		h: number,
