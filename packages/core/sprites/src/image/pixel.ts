...
export function getRGB(context: CanvasRenderingContext2D, x: number, y: number, result: RGBColor = { R: 0, G: 0, B: 0 }): RGBColor {
...
export function getRGBA(context: CanvasRenderingContext2D, x: number, y: number, result: RGBAColor = { R: 0, G: 0, B: 0, A: 255 }): RGBAColor {
	const d = context.getImageData(x, y, 1, 1);
	result.R = d.data[0];
	result.G = d.data[1];
	result.B = d.data[2];
