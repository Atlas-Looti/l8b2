...
export class Parser {
	...
	parseExpression(filter?: string | null, first_function_call: boolean = false): Statement | null {
	...
	assertExpression(filter?: string | null, first_function_call: boolean = false): Statement {
	...
	parseExpressionSuffix(expression: Statement, filter?: string | null): Statement | null {
		let field: Statement;
	...
	parseBinaryOperation(operation: Token, term1: Statement): Statement {
		const ops: Array<{ token: Token; operation: string }> = [{ token: operation, operation: operation.value as string }];
	...
	parseSelfAssignment(token: Token, expression: Statement, operation: number): SelfAssignment {
		if (!(expression instanceof Variable || expression instanceof Field)) {
			throw "Expected variable identifier or property";
		}
	...
	parseLocalAssignment(local: Token): Assignment {
		...
		return new Assignment(local, new Variable(identifier, identifier.value as string), this.assertExpression(), true);
	}
	...
	parseDelete(del: Token): Delete {
		...
		if (v == null || !(v instanceof Variable || v instanceof Field)) {
			return this.error("expecting variable name or property access after keyword `delete`") as Delete;
		} else {
