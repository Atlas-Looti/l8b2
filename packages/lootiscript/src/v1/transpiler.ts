...
export class Transpiler {
	...
	private readonly opcodeHandlers: Record<
		string,
		...
		for (let i = 0; i < r.opcodes.length; i++) {
			...
			if (this.transpilable(op, r.arg1[i])) {
				...
				while (
					...
					r.removeable(j) &&
					this.transpilable(
						OPCODES[r.opcodes[j] as keyof typeof OPCODES] as string,
						r.arg1[j],
					)
				) {
					j += 1;
	...
	private transpileSegment(r: Routine, i: number, j: number): void {
		...
		this.variables = {};
		s =
			"f = function(stack,stack_index,locals,locals_offset,object,global) {\n";
		for (k = i; k <= j; k++) {
	...
	private STORE_VARIABLE(arg: string): string {
		if (this.variables![arg] != null) {
			...
			return (
				'object["' + arg + '"] = ' + this.stack!.get() + " ; // STORE_VARIABLE"
			);
		}
	}
