	...
	import(src: {
		...
		for (let i = 0; i < this.opcodes.length; i++) {
			if (this.opcodes[i] === 100) {
				this.arg1[i] = (Compiler as any).predefined_unary_functions[
					this.arg1[i]
				];
			} else if (this.opcodes[i] === 101) {
				this.arg1[i] = (Compiler as any).predefined_binary_functions[
					this.arg1[i]
				];
			} else if (
				typeof this.arg1[i] === "object" &&
				!Array.isArray(this.arg1[i])
			) {
