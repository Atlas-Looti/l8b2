 ...
 */
export function handleRuntimeLogRequest(
	req: IncomingMessage,
	res: ServerResponse,
): boolean {
...
function emitRuntimeLog(payload: RuntimeLogPayload): void {
	const level =
		typeof payload.level === "string" ? payload.level.toLowerCase() : "info";
	const scope =
		typeof payload.scope === "string" ? payload.scope.toUpperCase() : "GAME";
